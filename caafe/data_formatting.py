...
# 等跑完了改成目标目录
output_path = "/home/jiahe/ML/Self_instruct_CAAFE/LLaMA-Factory/data/caafe_finetuning_data2.json"
change_prompt = {}
output_data = []
...
for line in lines:
    ...
    if float(original_data["improvement_roc"]) > 0 and float(original_data["improvement_acc"]) > 0:
        print(len(original_data["messages"][0]["content"])+len(original_data["messages"][1]["content"])+len(original_data["messages"][2]["content"]))
        if len(original_data["messages"][0]["content"])+len(original_data["messages"][1]["content"])+len(original_data["messages"][2]["content"]) < 3750:
            i = i + 1
